class Solution:
    ...
    def twoSumLinearithmicTimeAndLinearSpace(self, numbers: List[int], target: int) -> List[int]:
        for i in range(0, len(numbers)):
            remaining = target - numbers[i]
            j = self.binarySearchIterative(i + 1, len(numbers) - 1, numbers, remaining)
            if j != -1:
                return [i + 1, j + 1]

    def twoSum(self, numbers: List[int], target: int) -> List[int]:
        return self.twoSumLinearithmicTimeAndLinearSpace(numbers, target)